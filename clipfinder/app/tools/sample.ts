...
try {
  ...
  const hasVideo = (await $`ffprobe -v error -select_streams v:0 -show_entries stream=codec_type -of csv=p=0 ${source}`.quiet().nothrow().text()).trim() === "video";
  ...
  for (const [i, e] of excerpts.entries()) {
    ...
    if (hasVideo)
      await $`ffmpeg -v error -ss ${e.startS} -t ${EXCERPT_S} -i ${source} -vf scale=640:-2,fps=25 -c:v libx264 -profile:v main -preset slow -crf 30 -pix_fmt yuv420p -g 50 -ac 1 -ar 32000 -c:a aac -b:a 48k -af ${fade} ${p}`.quiet();
    else await $`ffmpeg -v error -ss ${e.startS} -t ${EXCERPT_S} -i ${source} -vn -ac 1 -ar 32000 -c:a aac -b:a 48k -af ${fade} ${p}`.quiet();
    parts.push(p);
  }
  ...
  await $`ffmpeg -v error -y -f concat -safe 0 -i ${listFile} -c copy -movflags +faststart ${audioOut}`.quiet();
  ...
  if (hasVideo && lead) await $`ffmpeg -v error -y -ss ${lead.startS + 3} -i ${source} -frames:v 1 -vf scale=960:-2 -q:v 4 ${join(outDir, "poster.jpg")}`.quiet();
