...
# The studio page's Vertical card plays Vertical's own sample: copied here at build time, so a replaced sample updates both pages.
mkdir -p dist/studio-demo && cp ../../vertical/app/dist/sample.mp4 dist/studio-demo/vertical-sample.mp4 && cp ../../vertical/app/dist/sample-track.json dist/studio-demo/vertical-track.json && cp ../../vertical/app/dist/poster.jpg dist/studio-demo/vertical-poster.jpg
...
sed -i -E "s#(sample\\.mp4|poster\\.(jpg|webp)|sample-words\\.json|vertical-track\\.json)(\\?v=[0-9]+)?([\"')])#\\1?v=$V\\4#g" dist/index.html dist/studio.html
