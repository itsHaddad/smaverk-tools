...
const RAIL = new URLSearchParams(location.search).get("rail") === "sandbox"
  ? { api: "https://sandbox-api.polar.sh", org: "655c19e9-2e14-40b6-9adf-612ca8937b49", link: "https://sandbox-api.polar.sh/v1/checkout-links/polar_cl_cr0egFs5EiYRoVHF9bm5AZghunw0Lnoe43szJ0ptTkQ/redirect" }
  : { api: "https://api.polar.sh", org: "d3095233-7f56-42d9-9cb5-cbc391af654c", link: "https://buy.polar.sh/polar_cl_EmfkZumlwFomZ7kIizVE6Xh0WnY0AEmn8LaS10NaemR" };
...
const UNLOCK = "https://unlock.smaverk.com"; // tiny worker: checkout id → the buyer's key (Polar's org token lives there, never in the page)
const KEY_STORE = "smaverk.captions.key";
...
// ---------- paid version: key from Polar ----------
async function validateKey(key: string): Promise<boolean> {
  if (!RAIL.org) return /^[A-Z0-9-]{16,}$/i.test(key) && key.startsWith("SMV"); // until the Polar product exists, keys are not issued; this only keeps the UI honest
  ...
  if (!r.ok) return false; const j = await r.json(); return j.status === "granted";
}
function setLicensed(on: boolean, msg?: string) {
  ...
  trust.textContent = on ? "Up to five minutes. English. Nothing leaves your device." : "Free up to 60 seconds. English. Nothing leaves your device.";
}
...
async function loadLicense() {
  // Straight back from Polar after paying: the URL carries a customer session token, so the key is fetched and applied here, nothing to paste.
  ...
  if (session || checkout) {
    const thanks = "Thank you. The paid version is on this device now. Your key is in the email from Polar for your other devices.";
    const fallback = "Payment received. Your key is in the email from Polar; paste it here to unlock this device.";
    setLicensed(false, "Payment received. Fetching your key…");
    let k = "";
    try {
      if (session) { const r = await fetch(`${RAIL.api}/v1/customer-portal/license-keys/?limit=10`, { headers: { authorization: `Bearer ${session}` } }); const j = r.ok ? await r.json() : null; k = (j?.items ?? []).find((x: any) => x.status === "granted" && x.key)?.key ?? ""; }
      if (!k && checkout) for (let i = 0; i < 6 && !k; i++) { if (i) await new Promise((r) => setTimeout(r, 2000)); const r = await fetch(`${UNLOCK}/key?checkout_id=${encodeURIComponent(checkout)}${RAIL.api.includes("sandbox") ? "&rail=sandbox" : ""}`); const j = r.ok ? await r.json() : null; if (j?.status === "granted" && j.key) k = j.key; else if (j?.status && !["pending", "confirmed", "succeeded"].includes(j.status)) break; }
    } catch {}
    if (k) { try { localStorage.setItem(KEY_STORE, k); } catch {} setLicensed(true, thanks); } else setLicensed(false, fallback);
    for (const p of ["customer_session_token", "paid", "checkout_id"]) u.searchParams.delete(p); history.replaceState(null, "", u.pathname + (u.search || "")); $("keystatus").scrollIntoView({ block: "center", behavior: "smooth" });
    return;
  }
...
$("keygo").addEventListener("click", async () => {
  ...
  try { if (await validateKey(k)) { try { localStorage.setItem(KEY_STORE, k); } catch {} setLicensed(true); } else { $("keystatus").textContent = "That key did not work. Check the email from Polar, or write to hello@smaverk.com."; $("keystatus").className = "fine err"; } }
